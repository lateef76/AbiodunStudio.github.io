...
document.addEventListener("DOMContentLoaded", function () {
  ...
  gsap.utils.toArray(".masonry-item").forEach((item, i) => {
    gsap.from(item, {
      scrollTrigger: { trigger: item, start: "top bottom-=50" },
      opacity: 0,
      y: 40,
      duration: 0.8,
      delay: i * 0.1,
  ...
  // Parallax video
  const projectsVideo = document.querySelector("#projectsVideo");
  if (projectsVideo) {
    window.addEventListener("scroll", () => {
      projectsVideo.style.transform = `translateY(${window.scrollY * 0.1}px) scale(1.05)`;
    });
  }
});
