...
document.addEventListener("DOMContentLoaded", function () {
  ...
  gsap.from(".philosophy-text", {
    scrollTrigger: { trigger: ".philosophy", start: "top bottom-=100" },
    opacity: 0,
    x: -50,
    duration: 1.2,
  });
  ...
  gsap.utils.toArray(".testimonial-card").forEach((card, i) => {
    gsap.from(card, {
      scrollTrigger: { trigger: ".testimonials-grid", start: "top bottom-=50" },
      opacity: 0,
      y: 30,
      duration: 0.8,
      delay: i * 0.1,
  ...
  if (homeVideo) {
    window.addEventListener("scroll", () => {
      homeVideo.style.transform = `translateY(${window.scrollY * 0.15}px) scale(1.05)`;
    });
  }
});
